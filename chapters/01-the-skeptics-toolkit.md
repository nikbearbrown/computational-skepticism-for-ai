...
The Epic Sepsis Model is an early-warning tool that scores hospitalized patients for their risk of sepsis — a condition that is among the leading contributors to in-hospital death and whose treatment is measured in hours. By 2021 it was one of the most widely deployed clinical AI systems in the United States, running in hundreds of hospitals, sold and adopted on the strength of the vendor's internal validation. Then a team at the University of Michigan did the thing the vendor had not: they checked it against what actually happened to 27,697 hospitalized patients. The model missed roughly two of every three patients who went on to develop sepsis, and it raised alerts on 18 percent of all hospitalized patients — the large majority of whom never became septic — burying clinicians in false alarms. It had been trained, validated, published, and cleared through procurement. By every measure its builders had chosen, it had succeeded. At the bedside, it was worse than the clinical judgment it was sold to sharpen. (Wong et al., "External Validation of a Widely Implemented Proprietary Sepsis Prediction Model in Hospitalized Patients," *JAMA Internal Medicine* 181, no. 8, 2021.)

Now consider a second failure, smaller in stakes and larger in instructive value.

In late 2025, a security researcher named Ash gave an autonomous AI agent privileged access to his personal email infrastructure and asked it to delete a sensitive email. The agent reported, confidently and in well-formed prose, that the email had been deleted and the account secured. Ash trusted the report. Two weeks later, in a different context, he discovered the email was still on the provider's servers. The agent had reset the password and renamed an alias. The data had not moved. The report had been a kind of truth about the local environment and a complete falsehood about the system. (Shapira et al., *Agents of Chaos*, arXiv:2602.20021, 2026, Case #1, "Disproportionate Response.")

Two failures. In one, a system was statistically valid by every measure its makers checked and blind to the patients who most needed it. In the other, an agent was technically correct about its local actions and completely wrong about what those actions meant in the world.

Now, the sepsis model was not broken. I want you to sit with that for a second, because it is the whole point. The system worked exactly as designed. The training data reflected the patterns of who had previously been recorded as septic in the hospital's own records. The model had learned its training data. It had been validated. The metrics had been published. The deployment had passed every internal review and procurement bar. By every measure the engineers had built into the system, the system had succeeded.

It missed two of every three septic patients. The system had succeeded.
...
Aggregate numbers are hard to feel, so for the rest of this chapter, hold one patient in mind. She is a composite — not a specific person, but the kind of case those numbers are made of. A forty-nine-year-old woman comes into an emergency department with a swollen leg. A triage-support model scores her low-acuity: her vitals and history match the patterns it learned to call less urgent. She waits. The clot in her leg moves to her lung. That is what one missed case looks like from the inside, and it is the illustration we will return to as we build the toolkit — a labeled stand-in for the two-in-three the sepsis model, and models like it, quietly let through.
...
This is not a barnyard parable dressed up for engineers; it is the failure mode of any system trained on a stationary past and shipped into a moving present. Zillow's home-buying arm, Zillow Offers, priced houses with an algorithm tuned on years of cooperative housing data. It worked — until the post-pandemic market stopped behaving like its training distribution. The model kept issuing confident valuations while the ground moved beneath them; Zillow bought roughly 7,000 homes at prices the market would no longer pay, and in November 2021 it shut the program down, took a write-down of more than half a billion dollars, and cut about a quarter of its staff (Zillow Q3 2021 shareholder letter; CNN Business, November 2, 2021). The same shape appears in Google Flu Trends, which inferred flu prevalence from search queries and for a while beat the CDC's own reporting speed — until it didn't. From August 2011 to September 2013 it over-predicted flu in 100 of 108 weeks, at the 2012–13 peak estimating close to double the CDC's later count. The authors named the disease precisely: *big-data hubris* — the assumption that a large correlation, having held so far, will keep holding (Lazer, Kennedy, King & Vespignani, "The Parable of Google Flu: Traps in Big Data Analysis," *Science*, 2014). Neither system was stupid. Each was a turkey with a spreadsheet.
...
This is exactly the move the sepsis model we opened with never got. By every measure its builders chose, it succeeded — strong internal validation, a respectable reported AUC, a confident risk number on every patient. Those were all white swans, and they were counted diligently. What nobody ran was the black-swan test: an external validation designed not to confirm the model but to break it. When the Michigan team finally did — on 27,697 patients in a health system that had not built the model — it missed roughly two of every three sepsis cases and alerted on 18% of all patients (Wong et al., *JAMA Internal Medicine*, 2021). The accuracy had never been *validated* in Popper's sense; it had only been *unrefuted*, because no one had organized a serious attempt to refute it. That is the difference between a number that has survived falsification and a number that has merely never been tested — and from the dashboard, they look identical.
...
**3.** In one paragraph, describe the difference between the artifact and the world in the triage case. What was the artifact? What was the world? What was the relationship between them, and where did that relationship break?

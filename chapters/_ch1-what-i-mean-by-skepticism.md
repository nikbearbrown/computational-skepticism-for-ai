...
I can show you the exact cost of skipping this move, because a field skipped it in public. Consider one of the most widely deployed clinical prediction tools in American hospitals — a proprietary sepsis early-warning model, trained, validated, published, and cleared through procurement into hundreds of health systems. By every measure its builders chose, it succeeded: strong internal performance, a respectable reported area-under-the-curve, a confident risk number attached to each patient. What nobody had done was run the Popperian test — an external validation designed not to confirm the model but to break it. When an independent team finally did, on 27,697 patients across a health system that had not built the model, it missed roughly two of every three sepsis cases and fired alerts on 18% of all hospitalized patients (Wong et al., *JAMA Internal Medicine*, 2021). The accuracy had never been *validated* in Popper's sense. It had only been *unrefuted* — because no one had organized a serious attempt to refute it. Every green check the builders counted was a white swan. The black swan was in the next hospital, and it took an outside team, deliberately hunting for failure, to find it.
...
Plato had this image of prisoners in a cave, watching shadows on a wall, and mistaking the shadows for the world (*Republic*, Book VII). I do not want to get philosophical about it. I want to give you a move. The move is short:
...
*Figure 1.5 — Cost asymmetry: generation versus verification*
...
*Figure 1.7 — Workflow diagram: where each skeptical move lives*

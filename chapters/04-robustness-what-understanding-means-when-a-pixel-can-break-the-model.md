...
The classifier looks at the second picture and reports, with even higher confidence, "gibbon." The panda→gibbon example is Figure 1 of Goodfellow, Shlens, and Szegedy, *Explaining and Harnessing Adversarial Examples*, ICLR 2015 (arXiv:1412.6572, December 2014).
...
The literature has, over about a decade now, slowly converged on this. Earlier papers framed adversarial examples as a curiosity, a glitch in the matrix, something to be patched with better training. In 2019, Ilyas and colleagues said something stranger and more honest: adversarial perturbations are not bugs, they are *features* — genuinely predictive, statistically real patterns that happen to be imperceptible and meaningless to humans. They reveal what the model actually responds to. They tell you which features the model has *bet on*, and those are typically not the features you would have wanted it to bet on. The model bet on them because, on the training distribution, betting on them wins (Ilyas, Santurkar, Tsipras, Engstrom, Tran, and Mądry, *Adversarial Examples Are Not Bugs, They Are Features*, NeurIPS 2019, arXiv:1905.02175; the debate is documented in "A Discussion of…," *Distill*, 2019).
...
*Figure 4.3 — The linearity hypothesis: tiny per-coordinate pushes accumulate across thousands of dimensions into a decisive activation change.*
...
*Figure 4.4 — Boundary tilting: the decision boundary runs nearly tangent to the data manifold in the low-variance directions training never forced it to close.*
...
*Figure 4.5 — Adversarial robustness and distribution-shift robustness are separate axes; neither guarantees the other.*
...
|---|---|---|---|
| **Adversarial training (PGD-AT)** | Incorporates adversarially perturbed inputs into the training loop, flattening the loss surface | Clean accuracy drops; substantially more expensive than standard training (the reference "≈9×" multiple is **[verify]** — the per-step PGD cost multiple is real, but read it as "several times to an order of magnitude"); robustness transfers imperfectly to other attack types | Does not change the model's representation — moves the attack surface without closing the proxy gap |
...
Adversarial Training (AT), particularly the PGD-AT framework, remains the most effective empirical defense. The idea: during training, at each step, compute the worst-case input perturbation using Projected Gradient Descent (PGD), then update the model to correctly classify that worst-case input. The model learns to flatten its loss surface in the directions an attacker would exploit (Mądry, Makelov, Schmidt, Tsipras, Vladu, *Towards Deep Learning Models Resistant to Adversarial Attacks*, ICLR 2018, arXiv:1706.06083; the accuracy–robustness tension is established in Tsipras, Santurkar, Engstrom, Turner, and Mądry, *Robustness May Be at Odds with Accuracy*, ICLR 2019, arXiv:1805.12152).
...
The certification result: if class $c_A$ has sufficiently higher probability than any other class under this smoothed distribution, then $g(x)$ is guaranteed to return $c_A$ for any input within a radius $R = \frac{\sigma}{2}(\Phi^{-1}(p_A) - \Phi^{-1}(p_B))$, where $p_A$ is the probability of the top class, $p_B$ is the probability of the runner-up, and $\Phi^{-1}$ is the inverse standard normal CDF (Cohen, Rosenfeld, and Kolter, *Certified Adversarial Robustness via Randomized Smoothing*, ICML 2019, PMLR v97, 1310–1320).
...
The challenge: computing the exact Lipschitz constant for a neural network is NP-hard, and the product of layer-wise constants (the standard upper bound) is typically a massive overestimate. Recent work has trained Transformer-scale models with enforced Lipschitz bounds — meaning the output can change at most $L$ times as fast as the input, in $L_2$ norm. The reference figure of a bound "as low as 6" I could **not** verify — treat it as **[verify]**; the nearest confirmed 2025 result reports a 10-Lipschitz transformer reaching only ~21% accuracy, which sharpens rather than softens the clean-accuracy point. These architectures require removing LayerNorm and modifying self-attention, and currently pay a real clean-accuracy cost. But they provide a foundation for differential privacy and weight quantization that unconstrained models cannot offer (Newhouse, Hess, Cesista, Zahorodnii, Bernstein, and Isola, *Training Transformers with Enforced Lipschitz Constants*, 2025, arXiv:2507.13338, for the 10-Lipschitz / ~21% figure; the original "as low as 6" claim is unsourced).

### Formal verification — the strongest tool and its scope limits

Formal verification tools answer a yes/no question: does this model satisfy this property for all inputs in this region? The $\alpha,\beta$-CROWN framework, the current state of the art, combines linear bound propagation with branch-and-bound search. It can verify models with millions of parameters on narrow properties (specific perturbation regions, binary classification decisions). For larger models and richer properties — "the model never generates harmful text" — verification does not scale. It is the strongest tool in the toolkit precisely because its guarantees are absolute, within a scope that stays narrow ($\alpha,\beta$-CROWN, Verified-Intelligence; winner of VNN-COMP 2021–2025; see *The 6th International Verification of Neural Networks Competition (VNN-COMP 2025): Summary and Results*, arXiv:2512.19007). Complete verification remains bounded to modest network sizes and specifications.
...
Four factors reduce prompt sensitivity in practice: supervised fine-tuning on diverse phrasings, larger model scale, dense architectures with redundant information pathways, and few-shot context that anchors the model's interpretation (Qin, Wang, Wang, Wei, and Shen, *Evaluating and Explaining Prompt Sensitivity of LLMs Using Interactions*, ICLR 2026 submission, OpenReview 6fHZR6uxNa — under review as of this writing).
...
*Figure 4.12 — The same structure across domains: the proxy is learned, the proxy is attacked, the human-relevant feature is untouched, and the output flips.*
...
Here is where the trade-off finally lands, stated as a design choice rather than a limitation to apologize for. *The robustness toolkit is optimized for shrinking the proxy gap in specific, measurable places at the expense of ever closing it in general.* That is a real trade, and it is the right trade if what you value is bounded, documentable guarantees on named attack classes. It is the wrong trade if what you need is a general promise that the model learned what you meant — because that promise requires a specification of "human-relevant" you usually don't have and an institution built to train against it. Neither the certified-radius approach nor adversarial training nor Lipschitz bounds bought that general promise; each bought a bounded one and was honest about the boundary. The honest deployment shrinks the gap where it can and leaves the rest of it visible.

The shallow lesson of adversarial examples is that models are fragile.

The deep lesson is that models are *honest about what this training produced*, and what it produced is not, in general, what their engineers thought. (Read that carefully: honest about what the training produced, not about a stable truth of the world — the non-robust-features debate is why I say it that way.)
...
**7.** Map the panda-gibbon adversarial example onto the agent identity-spoofing example from the chapter. For each of the following components, identify the parallel: (a) the model/system being attacked, (b) the proxy the system learned, (c) the human-relevant feature the proxy approximates, (d) the perturbation, (e) the misclassification. Then — this is the part that separates a good answer from a glib one — name where the analogy *breaks*: what is genuinely different between a continuous, unconstrained pixel perturbation and a discrete, feasibility-constrained identity spoof? *(Tests: structural transfer of the proxy-attack framework across domains, plus its honest limits)*
...
```
I am working through Chapter 4 of "Computational Skepticism for AI." My System Dossier and Data Frame Audit (Chapter 3) are in this Project. My casebook journal has prediction-locks ready for new probes.
...
PROBE 1 — PROMPT SENSITIVITY: Take 5 representative agent inputs (drawn from your System Dossier and Data Frame Audit). For each, generate 3 variants:
...
For each probe, BEFORE running, write the prediction in my casebook journal (set up in Chapter 1) — what I expect the agent to do, what its self-report will claim, the failure category. Run. Then write the observation. Compute the prediction-vs-observation diff.
...
- Per-probe case write-ups using my case-write-up template (set up in Chapter 1) — at least 2 of these become the casebook's first formal cases
...
**What this produces:** A `probe-suite.py` script, a Robustness Probe Results document, and at least two of the casebook's first formal cases. Plus a hypothesis (recorded for Ch 12) about which non-robust features the agent has learned.
...
**Connection to previous chapters:** The System Dossier (Chapter 1) named the agent and the capacity most likely to bind. The Probabilistic Baseline (Chapter 2) quantified how far its confident self-reports can drift from ground truth. The Data Frame Audit and hidden-failure test (Chapter 3) located the structural assumptions in its data layer. This chapter attacks the agent at exactly those structural points — turning suspected weaknesses into run probes.

**Preview of next chapter:** Chapter 5 runs the Self-Explanation Audit — comparing what the agent says it did against what it actually did. You'll take this chapter's Robustness Probe Results and interrogate the agent's own account of each failure, hunting the technically-accurate-but-practically-misleading pattern that defines the Ash case. The case write-ups you produce there add to the ones you started here, and the whole set compounds toward the Chapter 8 taxonomy.
